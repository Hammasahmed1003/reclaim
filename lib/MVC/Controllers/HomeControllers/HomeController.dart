...
// new fix with single Loader,

class HomeController extends GetxController {
  var isLoading = true.obs; // For affirmation loading
  var isSavedLoading = false.obs; // For saved lessons loading
  var affirmation = Rxn<Affirmation>();
  var savedLessons = <LessonModel>[].obs;
  RxBool showEveningButton = false.obs;
  ...
  Future<void> fetchSavedLessons() async {
    ...
    if (response != null && response.statusCode == 200 && response.data["error"] == false) {
      final List<dynamic> data = response.data["data"];
      savedLessons.value = data.map((item) => LessonModel.fromJson(item['lesson'])).toList();
    }
